...
class Workspace {
    ...
    addStand(id, visualId, label){
        const section = new Stand(id, visualId, label, this);
        this.stands.push(section);
        this._addToCanvas(section.getComponent());
    }
    ...
    removeSection(id) {
        ...
        this.sections.forEach(function (item, index) {
            if(item.id == id){
                sectionMetadata.item = item;
                sectionMetadata.index = index;
            }
        });

        this.canvas.remove(sectionMetadata.item.getComponent());
        this.sections.splice(sectionMetadata.index, 1);
